...
export default function QuestionnaireForm({
  ...
  return (
    <div>
      {section.title && (
        <Typography variant="h4" gutterBottom className={"!font-semibold !text-lg text-center !mb-4"}>
          {section.title}
        </Typography>
      )}
                ...
                >
                  {q.options.map((opt, idx) => (
                    <FormControlLabel
                      ...
                      key={idx}
                      value={opt}
                      control={<Radio color="primary"   size={'small'} disableRipple disableTouchRipple disableFocusRipple />}
                      label={opt}
                    />
                  ))}
                </RadioGroup>
